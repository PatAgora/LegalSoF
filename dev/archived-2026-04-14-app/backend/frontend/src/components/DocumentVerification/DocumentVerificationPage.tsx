...
export default function DocumentVerificationPage({ matterId }: Props) {
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-zinc-600">Loading verifications...</p>
        </div>
      </div>
    );
  }
  ...
  if (verifications.length === 0) {
    return (
      <div className="bg-white border border-zinc-200 rounded-md p-8 text-center">
        <svg className="w-12 h-12 text-zinc-400 mx-auto mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        </svg>
        <p className="text-sm text-zinc-600">No documents have been verified yet.</p>
        <p className="text-xs text-zinc-400 mt-1">Upload documents in the SoF Assessment tab to run verification.</p>
      </div>
    );
  }
  ...
  const verdictConfig = (verdict: string) => {
    switch (verdict) {
      ...
      default:
        return { label: verdict, bgClass: 'bg-zinc-50', textClass: 'text-zinc-900', borderClass: 'border-zinc-200', cardBg: 'bg-white' };
    }
  };
      ...
      <div className="bg-white border border-zinc-200 rounded-md p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-semibold text-zinc-900">Document Verification</span>
          </div>
          <div className="flex items-center gap-3 text-xs">
            <span className="text-zinc-600">{verifications.length} document{verifications.length !== 1 ? 's' : ''}</span>
            {verified > 0 && <span className="text-green-700 font-medium">{verified} passed</span>}
            {suspicious > 0 && <span className="text-amber-700 font-medium">{suspicious} review</span>}
            {tampered > 0 && <span className="text-red-700 font-medium">{tampered} failed</span>}
          </div>
        </div>
      </div>
        ...
        {verifications.map((v) => {
          ...
          return (
            <button
              ...
              className={`${vc.cardBg} border ${vc.borderClass} rounded-md p-4 text-left hover:shadow-lg transition-shadow group cursor-pointer`}
            >
              {/* File icon + name */}
              <div className="flex items-start gap-3 mb-3">
                {fileIcon(v)}
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-semibold text-zinc-900 truncate group-hover:text-zinc-900 transition-colors">
                    {v.filename}
                  </div>
