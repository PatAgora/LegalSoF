...
class DocumentVerifier:
    ...
    def _verify_property_claim(
        ...
        # 2. Confidence is 100% (no issues found)
        # Guard against division by zero when expected_amount is 0 or missing
        amount_matches = (
            net_proceeds is not None and
            expected_amount > 0 and
            abs(net_proceeds - expected_amount) / expected_amount < 0.01
        )
        result['verified'] = (
            amount_matches and
            bank_name is not None and
        ...
        result['verification_details']['comparison'] = {
            ...
            'matches': {
                'amount_matches': net_proceeds is not None and expected_amount > 0 and abs(net_proceeds - expected_amount) / expected_amount < 0.01,
    ...
    def _verify_business_claim(
        ...
        if doc_amount:
            # Allow 1% tolerance (guard against division by zero)
            amount_match = expected_amount > 0 and abs(doc_amount - expected_amount) / expected_amount <= 0.01
            if amount_match:
    ...
    def _verify_gift_claim(
        ...
        if not gift_amount:
            ...
            if expected_amount > 0:
                ...
                amount_diff = 1.0  # Treat as mismatch if expected_amount is 0
        ...
        result['verification_details']['comparison'] = {
            ...
            'matches': {
                'amount_matches': gift_amount and expected_amount > 0 and abs(gift_amount - expected_amount) / expected_amount <= 0.01,
    ...
    def _verify_savings_claim(
        ...
        # Check both supporting_docs AND bank_statements for a savings account
        savings_doc = None
        savings_account_found = False
        savings_account_id = None
        
        # First, check supporting_docs for savings-related documents
        for doc in supporting_docs:
            filename = doc.get('filename', '').lower()
            doc_type = doc.get('document_type', '').lower()
        ...
        # Second, check if there's a separate savings account in the bank_statements
        # Look for account_id containing 'savings', 'sav', 'isa' or a different account from the main one
        account_ids = set()
        for txn in bank_statements:
            account_id = txn.get('account_id', '') or txn.get('customer_id', '') or ''
            if account_id and account_id.lower() not in ['unknown', 'pdf_statement', 'pdf_pattern', 'ai_extracted']:
                account_ids.add(account_id)
        
        # Check if any account looks like a savings account
        for account_id in account_ids:
            account_lower = account_id.lower()
            if any(keyword in account_lower for keyword in ['saving', 'sav', 'isa', 'deposit']):
                savings_account_found = True
                savings_account_id = account_id
                break
        
        # If we have multiple accounts and one might be savings based on transfer descriptions
        if not savings_account_found and len(account_ids) > 1:
            # Look for transactions that indicate a savings transfer
            for txn in bank_statements:
                desc = (txn.get('description', '') or txn.get('narrative', '')).lower()
                if 'savings' in desc and ('transfer' in desc or 'from' in desc or 'to' in desc):
                    # This transaction references a savings account
                    savings_account_found = True
                    # Try to identify which account is the savings
                    for account_id in account_ids:
                        if account_id.lower() in desc or any(part in desc for part in account_id.lower().split()):
                            savings_account_id = account_id
                            break
                    break
        
        # Build the verification result
        if not savings_doc and not savings_account_found:
            ...
            result['differences'] = [
                ...
                },
                {
                    ...
                    'issue': 'Funds lineage verification required - trace funds from source account to destination',
                    'expected': 'Source documentation confirming the origin of funds transferred to current account',
                    'found': 'Use Funds Lineage Model tab to trace incoming credits and obtain source documentation'
                }
            ]
            return result
        
        # AUDIT TRAIL: Record which document/account was used for verification
        if savings_doc:
            result['verification_details']['document_used'] = {
        ...
        elif savings_account_found:
            result['verification_details']['document_used'] = {
                'filename': f'Bank statement for {savings_account_id}',
                'document_type': 'Savings Account Statement',
                'account_id': savings_account_id,
                'note': 'Savings account identified from uploaded bank statement transactions'
            }
        
        # For savings claims: Don't add placeholder differences here
        # The auto-lineage in the assessment endpoint will add actual results
        # We just mark that lineage analysis is needed
        if savings_account_found:
            issues.append("Savings account found - funds lineage analysis will run automatically")
            # Flag for lineage - actual differences will be added by auto-lineage
            result['lineage_required'] = True
            result['savings_account_id'] = savings_account_id
        else:
            issues.append("Savings claims require bank statements showing savings account")
            differences.append({
                'field': 'savings_documentation',
                'severity': 'missing',
                'issue': 'No savings account statement found - upload bank statement for savings account',
                'expected': f'Bank statements showing £{expected_amount:,.2f} savings accumulation',
                'found': 'No savings account transactions found in uploaded statements'
            })
        
        result['verified'] = False
        result['confidence'] = 0.4 if savings_account_found else 0.3
        result['issues'] = issues
        result['differences'] = differences
        result['requires_review'] = True
