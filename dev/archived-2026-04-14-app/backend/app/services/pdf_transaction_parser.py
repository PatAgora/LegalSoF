...
class PDFTransactionParser:
    ...
    def __init__(self):
        self.date_patterns = [
            ...
            r'\d{1,2}\s+[A-Za-z]{3}(?:\s+\d{2,4})?',  # DD MMM or DD MMM YY
        ]
    ...
    def parse_pdf(self, pdf_file: bytes, customer_id: str) -> List[Dict[str, Any]]:
        ...
        Uses the Universal Financial Parser for maximum compatibility
        with any bank format worldwide.
        ...
        try:
            ...
            print("\n🌐 Parsing PDF with Universal Financial Parser...")
            result = universal_parser.parse(pdf_file, filename='statement.pdf', content_type='application/pdf')
            ...
            if result['success'] and result['transactions']:
                ...
                for idx, txn in enumerate(result['transactions']):
                    ...
                    mapped_txn = {
                        ...
                        'country_iso2': 'GB',
                        'narrative': txn.get('description', '')[:500],
                        'channel': 'bank_transfer',
                        'source': txn.get('source', 'pdf_statement'),
                        'base_amount': float(txn.get('amount', 0)),
                    }
                    transactions.append(mapped_txn)
                ...
                print(f"⚠️ Universal parser found no transactions, trying legacy parser...")
                
        except Exception as e:
            import traceback
            traceback.print_exc()
            print(f"⚠️ Universal parser error: {e}, trying legacy parser...")
