...
class PDFDocumentExtractor:
    ...
    def _extract_property_completion_data(self, text: str, tables: List[List[List[str]]] = None) -> Dict[str, Any]:
        ...
        ENHANCED: Now handles table-based PDFs where labels and values 
        are in separate columns/lines
        ...
        print(f"\n=== COMPLETION STATEMENT EXTRACTION DEBUG ===")
        print(f"Text length: {len(text)} chars")
        print(f"Tables found: {len(tables) if tables else 0}")
        
        # ENHANCEMENT 1: Extract amounts from tables first
        # This handles PDFs where financial data is in table format
        table_amounts = []
        if tables:
            for table_idx, table in enumerate(tables):
                ...
                for row_idx, row in enumerate(table):
                    if row:
                        ...
                        # Look for amounts in each cell
                        for cell_idx, cell in enumerate(row):
                            if cell:
                                cell_str = str(cell).strip()
                                # Extract any monetary amount
                                amount_match = re.search(r'£([0-9,]+(?:\.\d{2})?)', cell_str)
                                if amount_match:
                                    amount = self._parse_amount(amount_match.group(1))
                                    if amount and amount > 0:
                                        # Check adjacent cells and row for context
                                        row_context = ' '.join([str(c) if c else '' for c in row]).upper()
                                        ...
                                        print(f"    Found amount: £{amount:,.2f} in context: {row_context[:80]}")
        
        # ENHANCEMENT 2: Find net proceeds from table data
        # Look for amounts on rows containing key phrases
        net_proceeds_keywords = ['NET PROCEEDS', 'NET AMOUNT', 'DUE TO YOU', 'BALANCE DUE', 'TOTAL DUE', 'PAYABLE TO']
        for amt_info in table_amounts:
            row_upper = amt_info['row_context'].upper()
            if any(kw in row_upper for kw in net_proceeds_keywords):
                data['net_proceeds'] = amt_info['amount']
                print(f"✅ Found net proceeds from table: £{amt_info['amount']:,.2f}")
                break
        
        # ENHANCEMENT 3: Find sale price from table data
        sale_price_keywords = ['SALE PRICE', 'CONTRACT PRICE', 'PURCHASE PRICE', 'AGREED PRICE', 'CONSIDERATION']
        if 'sale_price' not in data:
            for amt_info in table_amounts:
                row_upper = amt_info['row_context'].upper()
                if any(kw in row_upper for kw in sale_price_keywords):
                    data['sale_price'] = amt_info['amount']
                    print(f"✅ Found sale price from table: £{amt_info['amount']:,.2f}")
                    break
        
        # ENHANCEMENT 4: Multi-line pattern matching
        # Handle cases where label is on one line and value on next
        lines = text.split('\n')
        for i, line in enumerate(lines):
            line_upper = line.upper().strip()
            
            # Net proceeds - check current line and next line for amount
            if any(kw in line_upper for kw in ['NET PROCEEDS', 'NET AMOUNT DUE', 'DUE TO YOU', 'BALANCE DUE TO']):
                # Check same line first
                amount_match = re.search(r'£([0-9,]+(?:\.\d{2})?)', line)
                if amount_match and 'net_proceeds' not in data:
                    data['net_proceeds'] = self._parse_amount(amount_match.group(1))
                    print(f"✅ Found net proceeds on same line: £{data['net_proceeds']:,.2f}")
                # Check next line if no amount on current line
                elif i + 1 < len(lines) and 'net_proceeds' not in data:
                    next_line = lines[i + 1].strip()
                    amount_match = re.search(r'£([0-9,]+(?:\.\d{2})?)', next_line)
                    if amount_match:
                        data['net_proceeds'] = self._parse_amount(amount_match.group(1))
                        print(f"✅ Found net proceeds on next line: £{data['net_proceeds']:,.2f}")
            
            # Sale price - similar multi-line check
            if any(kw in line_upper for kw in ['SALE PRICE', 'CONTRACT PRICE', 'PURCHASE PRICE']):
                amount_match = re.search(r'£([0-9,]+(?:\.\d{2})?)', line)
                if amount_match and 'sale_price' not in data:
                    data['sale_price'] = self._parse_amount(amount_match.group(1))
                elif i + 1 < len(lines) and 'sale_price' not in data:
                    next_line = lines[i + 1].strip()
                    amount_match = re.search(r'£([0-9,]+(?:\.\d{2})?)', next_line)
                    if amount_match:
                        data['sale_price'] = self._parse_amount(amount_match.group(1))
        
        # ENHANCEMENT 5: Look for "transferred" or "payment of" sections
        # These often contain the net proceeds amount
        payment_section_match = re.search(
            r'(?:net proceeds|amount)\s+(?:of\s+)?£([0-9,]+(?:\.\d{2})?)\s+(?:has been|have been|were?)\s+(?:transferred|paid)',
            text, re.IGNORECASE
        )
        if payment_section_match and 'net_proceeds' not in data:
            data['net_proceeds'] = self._parse_amount(payment_section_match.group(1))
            print(f"✅ Found net proceeds from payment section: £{data['net_proceeds']:,.2f}")
        
        # Vendor name - comprehensive patterns
        vendor_patterns = [
            r'Seller[:\s]+(?:Mrs?|Miss|Ms|Dr)?\s*([A-Z][a-z]+\s+(?:[A-Z][a-z]+\s+)?[A-Z][a-z]+)',
        ...
        property_patterns = [
            ...
            r'Property[:\s]+(\d+[^,\n]+(?:,[^,\n]+){1,3})',  # Address starting with number
            r'PROPERTY[:\s]+(\d+[^,\n]+(?:,[^,\n]+){1,3})',
            r'Address[:\s]+(\d+[^,\n]+(?:,[^,\n]+){1,3})(?:\n|$)',
        ...
        for pattern in property_patterns:
            ...
            if match:
                ...
                if address and len(address) > 10:  # Sanity check - address should be reasonable length
        ...
        completion_patterns = [
            ...
            # Handle "15 October 2025" format
            r'(?:on|dated?)\s+(\d{1,2}\s+\w+\s+\d{4})',
        ]
        ...
        # Contract price / Sale price - original patterns (fallback)
        if 'sale_price' not in data:
            contract_patterns = [
                r'Sale Price[:\s]+£?([0-9,]+(?:\.\d{2})?)',
                r'Contract Price[:\s]+£?([0-9,]+(?:\.\d{2})?)',
                r'CONTRACT PRICE[:\s]+£?([0-9,]+(?:\.\d{2})?)',
                r'Purchase Price[:\s]+£?([0-9,]+(?:\.\d{2})?)',
            ]
            for pattern in contract_patterns:
                match = re.search(pattern, text, re.IGNORECASE)
                if match:
                    data['sale_price'] = self._parse_amount(match.group(1))
                    break
        
        # Net proceeds - original patterns (fallback)
        if 'net_proceeds' not in data:
            net_proceeds_patterns = [
                ...
                r'Balance due to you[:\s]+£?([0-9,]+(?:\.\d{2})?)',
            ]
        ...
        transfer_patterns = [
            r'Transfer Date[:\s]+(\d{1,2}(?:st|nd|rd|th)?\s+\w+\s+\d{4})',
            r'Payment Date[:\s]+(\d{1,2}(?:st|nd|rd|th)?\s+\w+\s+\d{4})',
            r'transferred.*?on\s+(\d{1,2}\s+\w+\s+\d{4})',
            r'paid.*?on\s+(\d{1,2}\s+\w+\s+\d{4})',
        ]
        ...
        # Bank name - stop at newlines to avoid capturing extra text
        bank_patterns = [
            r'Bank[:\s]+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*(?:\s+(?:Bank|PLC|plc))?)(?:\s*[\n\r]|$)',
            r'(Barclays|HSBC|Lloyds|NatWest|Santander|Halifax|Nationwide)(?:\s+Bank)?(?:\s*[\n\r]|$)',
        ]
        for pattern in bank_patterns:
            match = re.search(pattern, text, re.IGNORECASE)
            if match:
                bank_name = match.group(1).strip()
                # Remove any trailing text that shouldn't be there (like "Sort Code")
                bank_name = re.split(r'\s*(?:Sort|Account|A/C)', bank_name)[0].strip()
                if bank_name:
        ...
        # Sort code
        sort_code_match = re.search(r'Sort Code[:\s]+(\d{2}-\d{2}-\d{2})', text, re.IGNORECASE)
        if sort_code_match:
            data['sort_code'] = sort_code_match.group(1)
        
        # Transfer reference
        ref_patterns = [
            r'Payment Reference[:\s]+([A-Z0-9\s]+)',
        ...
        title_patterns = [
            r'Title Number[:\s]+([A-Z]{1,3}\d+)',
            r'TITLE NUMBER[:\s]+([A-Z]{1,3}\d+)',
            r'Title No\.?[:\s]+([A-Z]{1,3}\d+)',
            r'Land Registry Title[:\s]+([A-Z]{1,3}\d+)',
        ]
        ...
        solicitor_patterns = [
            r'(Harrison\s*[&]\s*Co(?:\s+Solicitors)?)',
            r'([A-Z][a-z]+\s+[&]\s+[A-Z][a-z]+(?:\s+Solicitors)?)',
            r'Solicitor[:\s]*([A-Z][a-z\s&]+(?:LLP|Solicitors)?)',
