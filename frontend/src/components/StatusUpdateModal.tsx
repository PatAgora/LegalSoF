...
export default function StatusUpdateModal({ 
  ...
  const fetchSuggestions = async () => {
    try {
      setLoading(true)
      const response = await fetch(`${API_BASE_URL}/api/v1/matters/${matterId}/status/suggestions`)
      if (!response.ok) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/v1/matters/${matterId}/status`, {
        method: 'PATCH',
        headers: {
  ...
  const getStatusColor = (status: string) => {
    const colors: { [key: string]: string } = {
      ...
      'CLIENT_UPLOADING': 'bg-cyan-100 text-cyan-800',
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="border-b border-gray-200 px-6 py-4">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-gray-900">Update Matter Status</h2>
            <button
              onClick={onClose}
